...
DOWNLOAD_FIRMWARE()
{
    ...
    if [ "$i" == "$SOURCE_FIRMWARE" ]; then
        ...
        curl -s --retry 5 --retry-delay 5 "https://27-samfw." -o "$ODIN_DIR/${MODEL}_${CSC}/firmware.zip"
        ...
        mkdir -p "$ODIN_DIR/${MODEL}_${CSC}"
        echo "- Downloading source firmware from custom URL..."
        curl -s --retry 5 --retry-delay 5 "https://27-samfw." -o "$ODIN_DIR/${MODEL}_${CSC}/firmware.zip"
