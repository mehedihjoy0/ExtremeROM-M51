...
if [ -n "$mod_url" ]; then
  ...
  BASE="$SRC_DIR/unica/mods/v4a"
  if [ -f "$EXTDIR/libv4a_re_armeabi-v7a.so" ]; then
    mkdir -p "$BASE/vendor/lib/soundfx"
    cp -f "$EXTDIR/libv4a_re_armeabi-v7a.so" "$BASE/vendor/lib/soundfx/libv4a_re.so"
    ADD_TO_WORK_DIR "$BASE" "vendor" "lib/soundfx/libv4a_re.so" 0 0 644 "u:object_r:vendor_file:s0"
  fi
  if [ -f "$EXTDIR/libv4a_re_arm64-v8a.so" ]; then
    mkdir -p "$BASE/vendor/lib64/soundfx"
    cp -f "$EXTDIR/libv4a_re_arm64-v8a.so" "$BASE/vendor/lib64/soundfx/libv4a_re.so"
    ADD_TO_WORK_DIR "$BASE" "vendor" "lib64/soundfx/libv4a_re.so" 0 0 644 "u:object_r:vendor_file:s0"
  fi
  rm -rf "$TMPDIR" "$EXTDIR"
fi
...
if [ -z "$V4A_APK" ]; then V4A_APK="https://github.com/WSTxda/ViperFX-RE-Releases/releases/download/7.2/viperfx-release.apk"; fi
